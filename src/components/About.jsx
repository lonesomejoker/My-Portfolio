...
const About = () => {
  return (
    ...
    viewport={{ once: false, amount: 0.25 }} className=' pt-[1rem] lg:pt-[3rem] pb-[2rem] bg-zinc-900'>
      <div className=' flex flex-col lg:gap-y-0 gap-y-4 lg:grid grid-cols-2 container items-center overflow-x-clip'>
      <div className=' space-y-4'>
        {
          aboutItems.map((item,idx)=>{
            return(
                <motion.div variants={fadeIn("right","tween",(idx+1)*0.3,1)} className=' px-3 lg:px-4 py-4 lg:py-6 flex items-center gap-x-5 border-2 border-stone-800 rounded-tr-2xl rounded-bl-2xl w-full lg:w-[80%] mx-auto lg:mx-0' key={item.id}>
      ...
      <motion.div variants={textVariant(0.5)} className=' font-[450] mt-4 lg:mt-0 '>
        ...
        <motion.div className=' flex gap-x-3 justify-center lg:justify-start'>
        <section  className=' border-[3px] border-stone-800 px-3 py-3 rounded-tr-2xl rounded-bl-2xl'>
            <h1 className=' font-[400] lg:font-600 text-[28px] lg:text-[32px] text-center'>8+ </h1>
            <p className=' text-center '>Projects Completed</p>
        </section>
        <section className=' border-[3px] border-stone-800 px-1.5 lg:px-3 py-3 rounded-tl-2xl rounded-br-2xl'>
